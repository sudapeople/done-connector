...
1. plugins 폴더에 done-connector-1.11.12.jar 파일을 넣고 마인크래프트 서버를 1회 실행 후 종료
...
* `/done ranking <스트리머 플레이어> [페이지] [all]` 스트리머의 후원 랭킹 조회. 페이지 번호를 생략하면 1페이지가 표시됩니다. 'all' 옵션 사용 시 테스트 후원도 포함하여 표시합니다. (자동완성 지원)
...
### 웹 대시보드 연동 (v1.11.9+)
웹서버 기반 완전한 활동 로그 시스템:

- **완전한 로그 추적**: 모든 관리자 작업이 웹 대시보드에 상세 기록
- **실시간 모니터링**: 인증/인증취소 등 모든 서버 관리 작업 실시간 추적
- **통합 로그 시스템**: 플러그인 활동과 관리자 활동 통합 표시
- **상세 정보 제공**: 시간, IP 주소, User Agent, 작업 상세 내용 완전 기록
